  ...
  if train_raw_file:
      ...
      with open(train_raw_file, 'r', encoding='utf-8') as f:
          for q in json.load(f)['questions']:
              ...
              # sort by length
              answers_dict[q['id']] = sorted(exact_answer,
  ...
  for entry in input_data:
    for paragraph in entry["paragraphs"]:
      ...
      if train_raw_file:
          ...
          # From 8B, answer masking
          paragraph_id = paragraph['qas'][0]['id'][:-4]
          paragraph_ans = paragraph['qas'][0]['answers'][0]['text']
          paragraph_ans_start = paragraph['qas'][0]['answers'][0]['answer_start']

          # tf.logging.info("paragraph_ans: " + paragraph_ans)
          # tf.logging.info("paragraph_ans_start: %d" % paragraph_ans_start)
          # tf.logging.info("paragraph_text: " + paragraph_text)

          paragraph_answers = answers_dict[paragraph_id]
          for pa in paragraph_answers:
              if pa == paragraph_ans:
                  continue
              if paragraph_ans in pa or pa in paragraph_ans:
                  continue
              if '52fb78d82059c6d71c000068' == paragraph_id \
                      and 'Ala735' in pa and 'Ala735' in paragraph_ans:
                  continue
              palen = len(pa)
              pstart = paragraph_text.find(pa, 0)
              while -1 < pstart:
                  paragraph_text = paragraph_text[:pstart] \
                                   + entity_tag \
                                   + paragraph_text[pstart + palen:]
                  if pstart < paragraph_ans_start:
                      paragraph_ans_start -= palen - len(entity_tag)
                  pstart = paragraph_text.find(pa, pstart + 1)

          # tf.logging.info("paragraph_ans: " + paragraph_ans)
          # tf.logging.info("shifted paragraph_ans_start: %d" % paragraph_ans_start)
          # tf.logging.info("masked paragraph_text: " + paragraph_text)
          # tf.logging.info(" ")

          paragraph['qas'][0]['answers'][0]['answer_start'] = paragraph_ans_start
